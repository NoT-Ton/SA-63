...
// QuerySells chains the current query on the sells edge.
func (uq *UserQuery) QuerySells() *BillQuery {
...
//  WithSells tells the query-builder to eager-loads the nodes that are connected to
// the "sells" edge. The optional arguments used to configure the query builder of the edge.
func (uq *UserQuery) WithSells(opts ...func(*BillQuery)) *UserQuery {
	query := &BillQuery{config: uq.config}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withSells; query != nil {
		...
		query.Where(predicate.Bill(func(s *sql.Selector) {
			s.Where(sql.InValues(user.SellsColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			...
			node.Edges.Sells = append(node.Edges.Sells, n)
		}
	}
