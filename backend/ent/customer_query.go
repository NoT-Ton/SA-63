...
// QueryBuys chains the current query on the buys edge.
func (cq *CustomerQuery) QueryBuys() *BillQuery {
...
//  WithBuys tells the query-builder to eager-loads the nodes that are connected to
// the "buys" edge. The optional arguments used to configure the query builder of the edge.
func (cq *CustomerQuery) WithBuys(opts ...func(*BillQuery)) *CustomerQuery {
	query := &BillQuery{config: cq.config}
...
func (cq *CustomerQuery) sqlAll(ctx context.Context) ([]*Customer, error) {
	...
	if query := cq.withBuys; query != nil {
		...
		query.Where(predicate.Bill(func(s *sql.Selector) {
			s.Where(sql.InValues(customer.BuysColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			...
			node.Edges.Buys = append(node.Edges.Buys, n)
		}
	}
