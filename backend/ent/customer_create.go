...
func (cc *CustomerCreate) AddBuyIDs(ids ...int) *CustomerCreate {
	cc.mutation.AddBuyIDs(ids...)
	return cc
}

// AddBuys adds the buys edges to Bill.
func (cc *CustomerCreate) AddBuys(b ...*Bill) *CustomerCreate {
	ids := make([]int, len(b))
	for i := range b {
		ids[i] = b[i].ID
	}
	return cc.AddBuyIDs(ids...)
}
...
func (cc *CustomerCreate) Save(ctx context.Context) (*Customer, error) {
	if _, ok := cc.mutation.CustomerName(); !ok {
		return nil, &ValidationError{Name: "customer_name", err: errors.New("ent: missing required field \"customer_name\"")}
	}
...
func (cc *CustomerCreate) createSpec() (*Customer, *sqlgraph.CreateSpec) {
	...
	if nodes := cc.mutation.BuysIDs(); len(nodes) > 0 {
